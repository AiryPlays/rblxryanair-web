...
async function handle(req) {
  ...
  if (pathName.startsWith('/admin/flight-details')) {
    if (req.method === 'GET') {
      ...
      // Efficient fetching for Supabase would use joins, but keeping it simple for now:
      const bookings = await db.get('bookings', { flight_id: fid });
      const bookingIds = bookings.map(b => b.id);
      
      // Fetch all and filter (inefficient for large DBs, okay for MVP)
      // For Supabase, we can use 'in' filter if we implemented it, but let's just fetch relevant rows
      // Ideally: db.get('passengers', { booking_id: 'in.(...)' })
      // For now, if using Supabase, we might fetch too much or need loop. 
      // Let's implement a simple loop for now or fetch all (limited)
