    ...
    def do_POST(self):
        ...
        if path == '/api/auth/register':
            ...
            try:
                ...
                uid = str(uuid.uuid4())
                cur.execute('INSERT INTO users (id, username, password_hash, role, created_at) VALUES (?, ?, ?, ?, ?)',
                           (uid, username, hashed, 'user', datetime.utcnow().isoformat()))
                conn.commit()
        ...
        if path == '/api/auth/login':
            ...
            cur = conn.cursor()
            hashed = hash_password(password)
            cur.execute('SELECT id, role FROM users WHERE username=? AND password_hash=?', (username, hashed))
            row = cur.fetchone()
            conn.close()
            if row:
                self.send_json({'success': True, 'id': row[0], 'role': row[1]})
            else:
